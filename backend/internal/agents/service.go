...
func decodeContainers(agent domain.Agent) []domain.ContainerSnapshot {
	if len(agent.Containers) == 0 {
		return nil
	}
	out := make([]domain.ContainerSnapshot, len(agent.Containers))
	copy(out, agent.Containers)
	return out
}

func (s *AgentService) Create(name, hostname, notes string, tlsEnabled bool) (*domain.Agent, error) {
	token := util.RandomString(48)
...
func (s *AgentService) GetByToken(token, ip string) (*domain.Agent, error) {
	if token == "" {
	...
	if agent.TokenBinding != "" && ip != "" {
		...
		_ = silentDB.Model(&domain.Agent{}).Where("id = ?", agent.ID).Update("token_binding", ip)
...
func (s *AgentService) Update(id, name, hostname, notes string, tlsEnabled bool) (*domain.Agent, error) {
	agent, err := s.Get(id)
	if err != nil {
		return nil, err
	}
...
func (s *AgentService) Delete(id string) error {
	return s.db.Delete(&domain.Agent{}, "id = ?", id).Error
}

func (s *AgentService) RotateToken(id string) (*domain.Agent, error) {
	agent, err := s.Get(id)
	if err != nil {
		return nil, err
	}
...
func (s *AgentService) CreateCommand(agentID, cmdType string, payload domain.JSONMap) (*domain.AgentCommand, error) {
	var agent domain.Agent
...
func (s *AgentService) GetNextCommand(agentID string) (*domain.AgentCommand, error) {
...
func (s *AgentService) UpdateCommand(cmd *domain.AgentCommand) error {
	return s.db.Save(cmd).Error
}

func (s *AgentService) UpdateCommandWithContext(ctx context.Context, cmd *domain.AgentCommand) error {
	return s.db.WithContext(ctx).Save(cmd).Error
}

func (s *AgentService) GetCommand(id, agentID string) (*domain.AgentCommand, error) {
	var cmd domain.AgentCommand
...
func (s *AgentService) ToggleContainerAutoUpdate(agentID, containerID string, enabled bool) error {
	...
	if !found {
	containers = append(containers, domain.ContainerSnapshot{
			ID:         containerID,
			AutoUpdate: enabled,
		})
	}
...
func ContainerDetailsFromReport(agent *domain.Agent, result domain.JSONMap, cmdPayload domain.JSONMap) (string, string, string) {
	containerID := ""
	name := ""
	image := ""
